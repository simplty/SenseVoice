    ...
    if ! command -v node &> /dev/null; then
        ...
        if [[ "$major_version" -lt 22 ]]; then
            ...
            if command -v nvm &> /dev/null || [[ -f "$HOME/.nvm/nvm.sh" ]]; then
                ...
                # 切换到v22
                if bash -c "source '$HOME/.nvm/nvm.sh' && nvm install 22 && nvm use 22"; then
...
install_nodejs() {
    ...
    if [[ "$OSTYPE" == "darwin"* ]] || [[ "$OSTYPE" == "linux-gnu"* ]]; then
        ...
        if timeout 300 bash -c "curl -o- https://raw.githubusercontent.com/nvm-sh/nvm/v0.40.3/install.sh | bash"; then
            ...
            if timeout 600 bash -c "source '$HOME/.nvm/nvm.sh' && nvm install 22"; then
                print_success "Node.js v22安装成功"
                
                # 重新加载nvm环境以确保node和npm可用
                source "$HOME/.nvm/nvm.sh"
                nvm use 22
                ...
                if command -v node &> /dev/null && command -v npm &> /dev/null; then
                    ...
                    print_error "安装后验证失败，请重新启动终端后再试"
                    exit 1
                fi
            else
                local exit_code=$?
